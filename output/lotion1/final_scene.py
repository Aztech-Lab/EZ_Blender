...
for mat in bpy.data.materials:
    if mat.use_nodes:
        ...
        # Find Principled BSDF and make the material pure gold metallic
        for node in nodes:
            if node.type == 'BSDF_PRINCIPLED':
                # Pure gold base color
                if 'Base Color' in node.inputs:
                    node.inputs['Base Color'].default_value = (1.0, 0.766, 0.336, 1.0)
                # Full metallic look
                if 'Metallic' in node.inputs:
                    node.inputs['Metallic'].default_value = 1.0
                # Low roughness for polished trophy finish
                if 'Roughness' in node.inputs:
                    node.inputs['Roughness'].default_value = 0.08
                # Keep specular reasonable for strong reflections
                if 'Specular' in node.inputs:
                    node.inputs['Specular'].default_value = 0.5
                # Optional: slightly reduce clearcoat unless already used
                if 'Clearcoat' in node.inputs:
                    node.inputs['Clearcoat'].default_value = 0.0
                if 'Clearcoat Roughness' in node.inputs:
                    node.inputs['Clearcoat Roughness'].default_value = 0.0


# --- lighting code ---

# --- Specular-friendly gold studio tuning ---
# Slightly reduce the HDRI so the controlled lights define the highlights more clearly.
bpy.data.worlds["World.001"].node_tree.nodes["HDRIHandler_ShaderNodeBackground"].inputs[1].default_value = 0.65

# Strengthen the right key light for crisp highlight bands.
area_light.data.energy = 14.2
area_light.data.color = (1.0, 0.96, 0.88)

# Keep the back wall light soft but a touch warmer for gold reflection support.
area_light001.data.energy = 9.5
area_light001.data.color = (1.0, 0.93, 0.82)

# Reduce the left fill slightly so the specular contrast stays elegant.
area_light002.data.energy = 0.85
area_light002.data.color = (1.0, 0.72, 0.58)

# Boost the top light to create a clean bright band across the surface.
area_light003.data.energy = 11.8
area_light003.data.color = (1.0, 0.98, 0.92)

# Add a compact warm specular kick from behind.
point_light.data.energy = 4.0
point_light.data.color = (1.0, 0.82, 0.55)


# --- background code ---

# --- Background simplification: make the world softer and less distracting ---
world = bpy.data.worlds.get("World.001")
if world is not None:
    world.use_nodes = True
    nodes = world.node_tree.nodes

    # Reduce environment brightness so the gold object remains the focal point.
    bg_node = nodes.get("HDRIHandler_ShaderNodeBackground")
    if bg_node is not None:
        bg_node.inputs[1].default_value = 0.35
        # Softer, slightly muted pink tint for the backdrop.
        bg_node.inputs[0].default_value = (0.95, 0.82, 0.88, 1.0)

    # If a world output exists, keep it connected as-is; no extra nodes needed.
